...
class ScanPipeFlagPipesTest(TestCase):
    ...
    def test_scanpipe_pipes_flag_read_file_head_text_normal(self):
        with tempfile.NamedTemporaryFile(suffix=".h", delete=False) as f:
            f.write(b"/* Generated by Cython 0.29 */\n#include <Python.h>\n")
            tmp_path = f.name

        result = flag.read_file_head_text(tmp_path)
        self.assertIn("generated by cython", result)
        self.assertEqual(result, result.lower())

    def test_scanpipe_pipes_flag_read_file_head_text_latin1(self):
        # A file with non-UTF-8 bytes should still return a string (latin-1 fallback).
        with tempfile.NamedTemporaryFile(delete=False) as f:
            f.write(b"autogenerated file\xff\xfe")
            tmp_path = f.name

        result = flag.read_file_head_text(tmp_path)
        self.assertIn("autogenerated file", result)

    def test_scanpipe_pipes_flag_read_file_head_text_missing_file(self):
        result = flag.read_file_head_text("/nonexistent/path/file.h")
        self.assertEqual("", result)

    def test_scanpipe_pipes_flag_read_file_head_text_null_bytes(self):
        # Null bytes (e.g. from a mostly-binary file header) should be stripped.
        with tempfile.NamedTemporaryFile(delete=False) as f:
            f.write(b"this file was generated by\x00\x00tool\n")
            tmp_path = f.name

        result = flag.read_file_head_text(tmp_path)
        self.assertNotIn("\x00", result)
        self.assertIn("this file was generated by", result)
