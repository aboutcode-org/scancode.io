...
class ScanPipeD2DPipesTest(TestCase):
    ...
    def test_scanpipe_pipes_d2d_get_basename_and_extension(self):
        filename1 = "somefilename1.d.ts"
        filename2 = "somefilename2.config.js"
        filename3 = "scripted.ts"

        basename1, extension1 = d2d._get_basename_and_extension(filename1)
        basename2, extension2 = d2d._get_basename_and_extension(filename2)
        basename3, extension3 = d2d._get_basename_and_extension(filename3)
    ...
    def test_scanpipe_pipes_d2d_source_content_in_map(self):
        ...
        result = d2d._source_content_in_map(to_resource, from_resource)
        self.assertEqual(True, result)

    def test_scanpipe_pipes_d2d_sha1(self):
        source_file = self.data_location / "d2d-javascript/from/main.js"
        with open(source_file) as f:
            source_text = f.read()
        result = d2d._sha1(source_text)
        self.assertEqual("d6bfcf7d1f8a00cc639b3a186a52453d37c52f61", result)

    def test_scanpipe_pipes_d2d_source_in_map(self):
        to_input_location = self.data_location / "d2d-javascript" / "to" / "main.js.map"
        to_dir = (
            self.project1.codebase_path / "to/project.tar.zst-extract/osgi/marketplace/"
            "intelligent robotics platform.lpkg-extract/"
        ...
        result = d2d._source_in_map(to_resource, "main.js")
        self.assertEqual(True, result)

    def test_scanpipe_pipes_d2d_source_mapping_in_minified(self):
        ...
        result = d2d._source_mapping_in_minified(
            to_resource, "sourceMappingURL=main.js.map"
        )
        self.assertEqual(True, result)

    def test_scanpipe_pipes_d2d_is_minified_and_map_compiled_from_source(self):
        to_dir = (
            self.project1.codebase_path / "to/project.tar.zst-extract/osgi/marketplace/"
            "intelligent robotics platform.lpkg-extract/"
            "com.example.adaptive.media.web-0.0.5.jar-extract/META-INF/"
        ...
        result = d2d._is_minified_and_map_compiled_from_source(
            to_resources, from_resource, ".js"
        )
        self.assertEqual(True, result)

    def test_scanpipe_pipes_d2d_relate_js_files(self):
        to_dir = (
            self.project1.codebase_path / "to/project.tar.zst-extract/osgi/marketplace/"
            "intelligent robotics platform.lpkg-extract/"
            "com.example.adaptive.media.web-0.0.5.jar-extract/META-INF/"
        ...
        d2d._relate_js_files(
            matches=to_resources,
    ...
    def test_scanpipe_pipes_d2d_map_javascript(self):
        ...
        expected = "Mapping 1 from/ resources using javascript map against to/ codebase"
        self.assertIn(expected, buffer.getvalue())
