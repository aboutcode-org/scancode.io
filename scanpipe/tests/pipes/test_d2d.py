...
class ScanPipeD2DPipesTest(TestCase):
    ...
    def test_is_probably_autogenerated_resource_cython_header(self):
        resource = make_resource_file(self.project1, path="to/lib.h")
        resource.location_path.parent.mkdir(parents=True, exist_ok=True)
        resource.location_path.write_text(
            "/* Generated by Cython 0.29 */\n#include <Python.h>\n"
        )
        self.assertTrue(d2d.is_probably_autogenerated_resource(resource))

    def test_is_probably_autogenerated_resource_autogenerated_file_marker(self):
        resource = make_resource_file(self.project1, path="to/setupconfig.py")
        resource.location_path.parent.mkdir(parents=True, exist_ok=True)
        resource.location_path.write_text(
            "# Autogenerated file for Kivy build\n# Do not modify by hand\n"
        )
        self.assertTrue(d2d.is_probably_autogenerated_resource(resource))

    def test_is_probably_autogenerated_resource_binary_skipped(self):
        resource = make_resource_file(self.project1, path="to/lib.so", is_binary=True)
        resource.location_path.parent.mkdir(parents=True, exist_ok=True)
        resource.location_path.write_bytes(b"\x7fELF\x02\x01\x01\x00" + b"\x00" * 56)
        self.assertFalse(d2d.is_probably_autogenerated_resource(resource))

    def test_is_probably_autogenerated_resource_plain_source_not_flagged(self):
        resource = make_resource_file(self.project1, path="to/main.c")
        resource.location_path.parent.mkdir(parents=True, exist_ok=True)
        resource.location_path.write_text(
            "#include <stdio.h>\nint main() { return 0; }\n"
        )
    ...
    def test_is_probably_autogenerated_resource_plain_source_not_flagged(self):
        ...
        src_file = to_dir / "main.c"
        src_file.write_text("#include <stdio.h>\nint main() { return 0; }\n")

        pipes.collect_and_create_codebase_resources(self.project1)
        resource = self.project1.codebaseresources.get(path="to/main.c")
        resource.update(is_text=True)
        self.assertFalse(d2d.is_probably_autogenerated_resource(resource))

    @mock.patch("scanpipe.pipes.d2d._match_purldb_resources")
    def test_match_unmapped_resources_autogenerated_reclassified(
        self, mock_match_purldb_resources
    ):
        """Autogenerated to/ files must be reclassified after REQUIRES_REVIEW is set."""
        mock_match_purldb_resources.return_value = True

        # Use make_resource_file to create DB records directly, avoiding the
        # bulk_create duplicate-ancestor issue from collect_and_create_codebase_resources.
        lib_h = make_resource_file(self.project1, path="to/lib.h")
        lib_h.location_path.parent.mkdir(parents=True, exist_ok=True)
        lib_h.location_path.write_text(
            "/* Generated by Cython 0.29 */\n#include <Python.h>\n"
        )

        main_c = make_resource_file(self.project1, path="to/main.c")
        main_c.location_path.write_text(
            "#include <stdio.h>\nint main() { return 0; }\n"
        )

        d2d.match_unmapped_resources(self.project1)

        lib_h.refresh_from_db()
        main_c.refresh_from_db()

        self.assertEqual(flag.IGNORED_NOT_INTERESTING, lib_h.status)
        self.assertEqual(flag.REQUIRES_REVIEW, main_c.status)
