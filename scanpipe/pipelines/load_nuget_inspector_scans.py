...
class LoadNugetInspectorScans(Pipeline):
    ...
    def get_nuget_inspector_scans(self):
        """Locate all the nuget inspector scan files from the project's input/ directory."""
    ...
    def load_nuget_inspector_scans(self):
        ...
        for resource_path in self.project.walk_codebase_path():
            resource, _ = CodebaseResource.objects.get_or_create(project=self.project, path=str(resource_path.stem))
            with open(resource_path) as f:
                json_data = json.load(f)
            packages = json_data.get("dependencies", [])
            for package_data in packages:
                update_or_create_package(self.project, package_data, codebase_resources=[resource])
