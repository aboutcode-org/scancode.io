...
def match_unmapped_resources(project, matched_extensions=None, logger=None):
    ...
    autogenerated_ids = [
        res.id
        for res in requires_review.iterator(chunk_size=2000)
        if is_probably_autogenerated_resource(res)
    ]
    if autogenerated_ids:
        project_files.filter(id__in=autogenerated_ids).update(
            status=flag.IGNORED_NOT_INTERESTING
        )


def is_probably_autogenerated_resource(resource):
    """
    Return True if `resource` looks like a generated file based on header markers.
    Only checks the first few KB of content.
    """
    ...
    head = flag.read_file_head_text(resource.location)
    if not head:
        return False

    return any(marker in head for marker in flag.AUTOGENERATED_FILE_MARKERS)
