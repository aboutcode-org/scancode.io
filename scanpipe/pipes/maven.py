...
def detect_maven_jars_from_pom_properties(project, logger_func=None):
    ...
    """
    if logger_func:
        logger_func("Detecting Maven JARs from pom.properties files and download URLs...")
    ...
    for pom_resource in pom_properties_resources:
        try:
            ...
            maven_coords = _extract_maven_coordinates_from_pom_properties(pom_resource)
            if not maven_coords:
                continue
                
            # Find the corresponding JAR package
            jar_package = _find_jar_package_for_pom(project, pom_resource, maven_coords)
            if not jar_package:
                continue
                
            # Update the package to use Maven PURL
            if _update_jar_package_to_maven(jar_package, maven_coords, logger_func):
                maven_jars_fixed += 1
    ...
    # Method 2: Look for JAR packages with Maven Central download URLs from input sources
        ...
        try:
            ...
            for input_source in input_sources:
                if input_source.download_url:
                    ...
                    # by matching filename or checking if the JAR was extracted from this source
                    potential_coords = _extract_maven_coordinates_from_url(input_source.download_url)
                    if potential_coords:
                        # Validate that this JAR package could be from this Maven coordinate
                        if _validate_maven_coordinates_against_jar_package(jar_package, potential_coords, input_source):
            ...
            if maven_coords:
                if _update_jar_package_to_maven(jar_package, maven_coords, logger_func):
                    maven_jars_fixed += 1
                    if logger_func:
                        logger_func(f"Converted JAR to Maven via input source URL: {input_source.download_url}")
...
def _extract_maven_coordinates_from_url(download_url):
    ...
    try:
        ...
        if not any(maven_host in parsed.netloc.lower() for maven_host in 
                  ['repo1.maven.org', 'central.maven.org', 'repo.maven.apache.org']):
            return None
        ...
        if filename != expected_filename:
            ...
            if not filename.startswith(f"{artifact_id}-{version}-") or not filename.endswith('.jar'):
                return None
...
def _extract_maven_coordinates_from_pom_properties(pom_resource):
    ...
    try:
        ...
        content = Path(pom_resource.location).read_text(encoding='utf-8', errors='ignore')
...
def _find_jar_package_for_pom(project, pom_resource, maven_coords):
    ...
    jar_packages = project.discoveredpackages.filter(
        type='jar'
    )
    ...
    for package in jar_packages:
        ...
        for resource in package_resources:
            if resource.path == jar_path or resource.path.startswith(jar_path + '-extract/'):
                # Additional validation: check if the Maven coordinates match what we expect
...
def _validate_maven_coordinates_against_jar_package(jar_package, maven_coords, input_source):
    """
    Validate that the Maven coordinates make sense for this JAR package
    based on the input source that provided the JAR.
    ...
    if input_source.filename:
        expected_jar_name = f"{maven_coords['artifact_id']}-{maven_coords['version']}.jar"
    ...
    if jar_package.name:
        ...
        if (maven_coords['artifact_id'] in jar_package.name or 
            maven_coords['group_id'] in jar_package.name):
            return True
    
    # Check if the namespace matches the group ID
    if jar_package.namespace and jar_package.namespace == maven_coords['group_id']:
        return True
...
def _update_jar_package_to_maven(jar_package, maven_coords, logger_func=None):
    ...
    try:
        ...
        logger.error(f"Failed to update package {jar_package.uuid} to Maven PURL: {e}")
        return False